...
| 사라진 프록시는 동기화를 실패시키지 않고 다시 시작함 | 구현됨 | `go test ./internal/stack -run 'TestAProxyThat\|TestAConfigurationTheProxy'`가 컨테이너가 없을 때 엔진이 쓰는 문구, 컨테이너 목록과 reload 사이에 제거된 프록시, 프록시가 거부하는 설정을 검사. 변경 전에는 첫 경우가 `container exec containerctl-edge nginx -s reload: Error: container with ID containerctl-edge not found`로 실패하며, 이 기계에서 `containerctl peer add`가 승인을 저장한 뒤 낸 오류가 그것 | 예 |

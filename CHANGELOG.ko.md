...
라우트가 그 이름을 제공하지 않으면 인증서를 미사용으로 불렀다. 정지된
프로젝트는 아무것도 제공하지 않으므로 그 인증서가 전부 미사용으로 보고되고
제거가 제안됐다. 다음 시작에 필요한 것들이다. 이제 라우트도, 등록된 프로젝트도
그 이름을 요구하지 않을 때만 미사용이다.

검증: `make check`. 라우트 없는 선언된 도메인, 라우트된 도메인, 대소문자를 검사한다.
이 머신에서 모든 프로젝트가 정지한 상태로 변경 전에는 `api.platform3.test`를
미사용이라 불렀고, 이제는 그것을 선언한 프로젝트를 표시한다.

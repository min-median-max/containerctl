...
| 다른 기계의 도메인에 닿기 | 구현되지 않음 | `docs/spec/peers.ko.md`가 설계와 그 근거가 된 측정을 서술한다. `container run -p 443:80`이 관리자 권한 없이 `*.443`에 붙었고, 주소만으로는 기본 인증서와 함께 404, 주소와 올바른 이름으로는 그 사이트, 프록시가 받아들이지 않는 주소에서는 같은 요청이 403이었다 | 아니오 |

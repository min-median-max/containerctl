...
| root는 필요한 자리에서만 요구 | 구현됨 | 이 기계에서 모든 도메인이 이미 위임된 상태로 `containerctl install`이 주인을 가져가고 에이전트를 다시 등록하는 동안 암호를 한 번도 묻지 않음. `doctor`는 `/etc/resolver`에 쓰는 단계에만 표시하고, 인증기관 신뢰는 키체인 변경이므로 더 이상 관리자 권한으로 적지 않음 | 예 |

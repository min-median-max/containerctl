...
다만 하나는 reload 실패로 보지 않는다. 프록시가 없다는 엔진의 응답이다. 프록시는
설정을 쓰기 전에 컨테이너 목록에서 읽고 reload는 그 뒤에 보내므로, 그 사이에
제거된 프록시는 방금 실행 중이라고 나열한 엔진이 없다고 답하게 된다. 이때는
지켜야 할 실행 중인 프록시도, 재시작하게 될 다른 프로젝트의 프록시도 없고 설정은
이미 쓰인 뒤이므로, 프록시를 시작하고 동기화 결과를 started로 보고한다. 그 밖의
reload 실패는 모두 그대로 반환한다. 프록시가 설정을 거부한 경우도 마찬가지다.
거기서 프록시를 교체하면 시작한 뒤 같은 이유로 실패하는 컨테이너 뒤에 원인이
가려진다.

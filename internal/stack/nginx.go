...
func RenderNginxConfig(confDir string, c NginxConfig) error {
	...
	if len(routes) == 0 && len(c.PeerRoutes) == 0 && c.PeerPort == 0 {
	...
	// nginx fails to start when the resolver directive has no address.
	if resolver != "" {
		fmt.Fprintf(&b, "resolver %s valid=%s ipv6=off;\n\n", resolver, heldAddress)
	}
	// An upgrade request sends Connection: upgrade. Every other request sends no
	// Connection header, because nginx omits a header whose value is empty.
	// Sending Connection: close makes the upstream end the response before the

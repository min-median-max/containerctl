...
| Window in English and Korean | Implemented | `go test ./internal/i18n` reads the window's sources and fails on a line with no Korean, a translation taking different values, or a rejected phrasing; with the system set to Korean the window was captured in Korean | Yes |

...
| 다른 기계의 도메인에 닿기 | 구현되지 않음 | `docs/spec/peers.ko.md`가 설계와 그 근거가 된 측정을 서술한다. 공개한 포트가 출발지 주소를 유지하지 않으므로 피어를 인증서로 알아본다. 인증서 없이는 nginx가 400, 승인된 인증기관이 발급한 것으로는 제공하며 피어를 밝혔다. 자기 기계의 인증기관만 신뢰하는 클라이언트가 피어의 내용을 받았으므로 어떤 인증기관도 다른 기계에 설치되지 않는다 | 아니오 |

...
| 정지된 프로젝트의 인증서를 미사용으로 부르지 않음 | 구현됨 | `go test ./internal/stack -run TestAStoppedProjectsCertificateIsNotOrphaned`가 라우트 없는 선언된 도메인, 라우트된 도메인, 대소문자를 검사. 변경 전에는 프로젝트가 정지했을 뿐인데 `api.platform3.test`를 미사용이라 부르고 제거를 제안했음. 이제 소유 프로젝트를 표시하고 재발급을 제안 | 예 |

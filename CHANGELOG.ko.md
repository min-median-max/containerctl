...
시스템 언어가 한국어면 창을 한국어로 표시한다. 영어는 호출 지점에 적는 문구이고
한국어는 그것으로 찾으므로, 번역이 없는 줄은 영어로 표시된다. 명령줄은 영어를
유지한다. 그 출력이 생성 문서의 근거인 명세이기 때문이다.

배치 오류 둘을 고쳤다. 위쪽 정렬 행이 두 줄짜리 값을 감싸도록 늘어나지 않아 둘째
줄이 카드 끝에서 잘렸고, 이제 값 아래 여백을 직접 명시한다. 출력 창의 한 줄이
내용 너비가 아니라 창 너비에 묶여 좌우 여백을 덮었다.

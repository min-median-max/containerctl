...
| 실패는 실패한 서비스의 출력을 싣는다 | 구현됨 | `go test ./internal/stack -run 'TestAFailure\|TestTheOutputCarried'`가 말을 남긴 서비스, 남기지 않은 서비스, 상한을 검사. 출력을 감추던 규칙은 아무것도 지키지 못했다. 같은 사람이 `containerctl logs` 한 번이면 같은 바이트를 보므로 비밀은 못 사고 증거만 잃었다. 이 기계에서 크래시한 데이터베이스가 healthcheck 중 멈췄다고만 보고됐고 크래시는 컨테이너 로그에 그대로 있었다 | 예 |
| 생성 실패는 런타임이 거부한 대상을 지목한다 | 구현됨 | `go test ./internal/stack -run 'TestACreationFailure\|TestEnvironmentValues\|TestRedactionDoes'`가 런타임의 문장, 이 명령이 전달한 모든 `KEY=VALUE` 가리기, 짧은 값처럼 읽히는 평범한 단어, 빈 진단을 검사. 환경 값은 명령줄에 실리므로 값이 아니라 쌍을 가린다. 이 기계에서 `up`이 `runtime rejected creation`이라고만 하던 것이 이제 `Error: path '/Users/maxkwon/orm/.runtime/db-tests' does not exist`로 나온다. 분류기가 런타임이 실제로 쓰는 `does not exist`를 몰랐다 | 예 |
| 프로젝트는 어떤 일보다 먼저 등록된다 | 구현됨 | `go test ./internal/stack -run 'TestUpRegisters\|TestEachSlowStep\|TestTheHealthcheckWait'`가 실패한 `up` 뒤의 등록부, 느린 단계를 끝난 뒤가 아니라 시작 전에 알리는 순서, healthcheck 대기가 상한을 밝히는지를 검사. 창은 등록부를 그리므로 일이 끝난 뒤 등록하면 긴 실행 내내 보이지 않고 실패하면 끝내 보이지 않았다 | 예 |
| 주인이 아닌 명령은 아무것도 바꾸기 전에 멈춤 | 구현됨 | `go test ./internal/stack -run 'TestUpStops\|TestDownStops\|TestServiceActionsStop'`가 up·down·start·stop·restart를 검사. 검사가 머신 설정 자리에 있었는데, `up`은 볼륨을 만들고 프로젝트를 등록한 뒤에, `down`은 컨테이너를 지운 뒤에 그 자리에 닿는다. `up` 테스트가 이미 등록된 프로젝트로 실패했다. 이제 각 동작의 첫 문장이다. 이 기계에서 두 번째 상태 디렉터리로 `sync`·`uninstall`·`down`을 돌리자 각각 주인을 말하며 멈췄고 아무것도 바꾸지 않았다 | 예 |

...
규칙은 진리가 아니다. 각 규칙은 조건 위에서 성립하고, 첫 규칙을 뺀 나머지는 바로
위 규칙이 강제한다. 조건이 더는 성립하지 않는 규칙은 그에 맞춘 코드를 쓰기 전에
여기서 다시 본다.

1. 컨테이너는 자기 엔진의 네트워크에서 닿는다. 호스트가 그 네트워크에 닿을 수
   있는지는 컨테이너가 아니라 엔진의 성질이다. 조건: 이 기계에서 Apple
   `container`는 호스트가 경로를 가진 서브넷에 컨테이너를 두고, Docker는 호스트가
   경로를 갖지 않는 브리지에 둔다. 호스트에 경로를 주는 엔진이 나오면 이 규칙과
   그 아래 모든 규칙이 바뀐다.

2. 다른 엔진의 컨테이너에 닿는 것에 의존해서는 안 된다. 느리거나 불안정한 것이
   아니라 되지 않는다.

3. 프록시는 엔진당 하나다. 프록시는 자기 엔진의 컨테이너를 담당한다. 규칙 2에
   따라 다른 것은 담당할 수 없기 때문이다.
...
5. 도메인은 머신당 한 번 점유된다. 점유는 어느 프록시를 설정하기 전에 모든
   엔진의 컨테이너를 대상으로 확정한다. 그래서 컨테이너가 양쪽에서 실행한다고
   이름을 두 번 가질 수 없고, 어느 한쪽에서 실행한다고 잃지도 않는다.

6. 이름은 그 이름의 컨테이너가 실행되는 엔진의 프록시 주소로 해석된다.

7. 머신이 네트워크에 내주는 포트는 하나이며 피어 링크다. 나머지는 loopback이나
   엔진 자신의 네트워크에 있다.

8. 피어 링크는 이 머신이 제공하는 모든 도메인을 제공한다. 규칙 2에 따라 어느
   엔진도 그럴 수 없으므로, 피어 링크는 각 엔진의 프록시로 넘기는 호스트
   프로세스다.

9. 관리자 권한이 필요한 것은 `/etc/resolver` 쓰기뿐이다. 피어 링크는 권한이
   필요 없는 8443을 잡으므로 규칙 8이 이를 바꾸지 않는다.

10. 어느 컨테이너도 주장하지 않는 이름과 피어의 도메인은 어느 엔진에도 속하지
    않는다. 모든 프록시가 이들을 제공하도록 설정하므로 답이 어느 쪽을 가리켜도
    옳고, 답은 존재하는 첫 엔진을 가리킨다.
